...
idl_path = "IDL_dataset_path_here.xlsx"
...
##########################2. LTL DATASET
path_data = "data_path_here.xlsx"
data = read_excel(path_data)
path_followup = "followup_data_path_here.xlsx"
...
ggplot(data = data, aes(x = log(ltl), fill = factor(sex))) +
  geom_density(alpha = 0.5) +
  labs(title = "Density Plot of log(ltl) by Sex", x = "Attrition", y = "Density") +
  scale_fill_manual(values = c("blue", "pink"), labels = c("Male", "Female"))

##SPLINE REGRESSION
model = lm(ltl ~ ns(Age, df = 3), data = data)
age_seq = seq(min(data$Age), max(data$Age), length.out = 300)
...
ggplot(data, aes(x = Age, y = ltl)) +
  ...
  labs(x = "Age", y = "LTL (T/S)") + theme_minimal()
...
data$AgeGroup = cut(age,
                    breaks = breaks_age,
                    include.lowest = TRUE,
                    right = FALSE)

km_fit_age = survfit(surv_object ~ AgeGroup, data = data)
